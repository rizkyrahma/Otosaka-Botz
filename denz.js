...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© Alam Botz_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
stod = `${sender}`
       stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🌹*ALAM BOTZ* 🌹'
       ownerJid = "6285888083107@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `╭─❒ 「 Bot Info 」 ❒
├ _Creator : @${dtod.split('@')[0]}_
...
└──────[ GITHUB ]──────❒
  │◦➛Script Ory :
  │◦➛https://github.com/Alam-Botz/Otosaka-Botz
  │◦➛Script Recode : 
  │◦➛https://github.com/Alam-Botz
  └──────────────────❒`
 buttons = [
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://github.com/Alam-Botz"
        },
        "index": 0
      }
    ]
...
if (args.length < 1) return reply('Jumlahnya?')
	    ...
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
						...
						"message": `${NamaBot}`,
						"orderTitle": `${NamaOwner}`,
						"sellerJid": "6285888083107@s.whatsapp.net",
						"token": "AR4QmUKv7r4P0XYHtHmhLqoFOOhwn8SqO903CVo9raQL4A=="
					}}}})
		        }
		        }
